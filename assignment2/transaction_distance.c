...
int transaction_commodity_distance(struct bot *b)
{
		...
		if(b->cargo==NULL)
	{
		...
		else
		{
			...
			{
				for(i=0, sell = b->location;i<abs(profit_sell(b));i++, sell=sell->previous)
				{
				}
			}
			...
			if(profit_sell(b) ==0)
			{
				ans = b->cargo->quantity;
				return ans;
			}
			else
			{
				return profit_sell(b);
			}
			if(dump(b) == 1)
			{
